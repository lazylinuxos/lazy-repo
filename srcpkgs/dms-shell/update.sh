...
LATEST_VERSION=$(gh release list --repo AvengeMedia/DankMaterialShell --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R AvengeMedia/DankMaterialShell --archive=tar.gz --output "${VERSION}.tar.gz"
