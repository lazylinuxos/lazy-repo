...
_FLAGS_FILE="${XDG_CONFIG_HOME}/antares-sql-flags.conf"
declare -a flags
...
if [[ "${EUID}" -ne 0 ]] || [[ "${ELECTRON_RUN_AS_NODE}" ]]; then
    exec electron35 "${_RUNNAME}" "${_OPTIONS}" "${flags[@]}" "${_USER_FLAGS}" "$@" || exit $?
else
    exec electron35 "${_RUNNAME}" "${_OPTIONS}" --no-sandbox "${flags[@]}" "${_USER_FLAGS}" "$@" || exit $?
fi