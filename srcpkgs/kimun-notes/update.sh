#!/usr/bin/env bash


printf "Checking latest version\n"

__dir="$(dirname "${BASH_SOURCE[0]}")"

GH_REPO="nico2sh/kimun"

LATEST_VERSION=$(
    gh release list \
        --exclude-pre-releases \
        --repo "${GH_REPO}" \
        --json tagName \
        --jq '.[] | select(.tagName | startswith("kimun-notes-v")) | .tagName' \
        | sort -V \
        | tail -n1
)
export VERSION="${LATEST_VERSION#kimun-notes-v}"
CURRENT_VERSION=$(grep -E '^version=' ${__dir}/template | cut -d= -f2)

printf "Latest version is: %s\nLatest built version is: %s\n" "${VERSION}" "${CURRENT_VERSION}"
[ "${CURRENT_VERSION}" = "${VERSION}" ] && printf "No new version to release\n" && exit 0

# No preprepped checksum files, need to download the binary and calculate it myself
gh release download -R "${GH_REPO}" "${LATEST_VERSION}" --archive=tar.gz --output "${VERSION}.tar.gz"
export SHA256=$(sha256sum ./${VERSION}.tar.gz | cut -d ' ' -f1 )
rm ./${VERSION}.tar.gz
[[ ! ${SHA256} =~ ^[a-z0-9]+$ ]] && printf "got junk instead of sha256\n" && exit 1

envsubst '${SHA256} ${VERSION}' < ${__dir}/.template > ${__dir}/template

printf "kimun-notes template updated\n"
