...
export SHA256=$(curl -L https://github.com/${GH_REPO}/releases/download/v${VERSION}/brave-origin-${VERSION}-1.x86_64.rpm.sha256 | cut -d ' ' -f1 )
[[ ! ${SHA256} =~ ^[a-z0-9]+$ ]] && printf "got junk instead of sha256\n" && exit 1

envsubst '${SHA256} ${VERSION}' < ${__dir}/.template > ${__dir}/template

printf "brave-origin-bin template updated\n"