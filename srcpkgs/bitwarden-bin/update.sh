...
LATEST_VERSION=$(gh release list --repo ${GH_REPO} --exclude-drafts --exclude-pre-releases --json tagName --jq '[.[] | select(.tagName | startswith("desktop-v"))][0].tagName')
export VERSION=${LATEST_VERSION#"desktop-v"}
...
ASSET="Bitwarden-${VERSION}-amd64.deb"
...
printf "bitwarden-bin template updated\n"